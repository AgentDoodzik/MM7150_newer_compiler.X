...
void config_DMA(void)
{
    ...
    if(IS_KVA0(tx_buff))
        source_address_kseg1 = (uint32_t)KVA0_TO_KVA1(tx_buff);
    else
        source_address_kseg1 = (uint32_t)tx_buff;
    ...
    DCH0DAT = '\0'; //termination mark - null - stops the transfer
    ...
    DCH0SSIZ= 100; // source size 20 bytes
